...
import { Users, Heart, Share, Send, ExternalLink } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";

interface CloudflareStreamViewerProps {
  streamId: string;
  title: string;
  streamerName: string;
...
export default function CloudflareStreamViewer({
  streamId,
  title,
  streamerName,
  streamerAvatar,
  viewerCount,
}: CloudflareStreamViewerProps) {
  ...
  const openInNewTab = () => {
    window.open(`https://cloudflarestream.com/watch/${streamId}`, '_blank');
  };
      ...
      <div className="lg:col-span-3 flex flex-col">
        ...
        <div className="relative bg-black rounded-lg overflow-hidden aspect-video flex-1">
          <video
            ...
            autoPlay
            muted
            poster="https://images.unsplash.com/photo-1611162617474-5b21e879e113?ixlib=rb-4.0.3&w=800&h=450&fit=crop"
          />
          ...
          {/* Cloudflare branding */}
          <div className="absolute bottom-4 right-4 bg-black/50 rounded px-2 py-1">
            <span className="text-white text-xs">Powered by Cloudflare Stream</span>
          </div>

          {/* Simulated stream overlay */}
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900/90">
            <div className="text-center text-white">
              <div className="text-lg font-medium mb-2">🎥 Cloudflare Live Stream</div>
              <p className="text-gray-300 mb-4">Professional streaming with global CDN</p>
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-center space-x-2">
                    <Avatar className="w-6 h-6">
                      <AvatarFallback className="text-xs bg-blue-500 text-white">CF</AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="font-medium text-blue-600">Cloudflare</div>
                      <p className="text-xs text-gray-400">Stream is powered by Cloudflare's global network</p>
                    </div>
                  </div>
                </div>
              </div>
